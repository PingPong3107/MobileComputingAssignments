...
def forward_message(udp_packet : bytes):
    ...
    event_logger(f"Forwarded message: {udp_packet} to {broadcast_address}:{broadcast_port}")
    sock.close()

def burst_message(socket:socket.socket, udp_packet:bytes, addr:str, port:int, burst_count: int = 5):
...
if __name__ == "__main__":
    ...
    test_message: bytes = generate_message(destination=sys.argv[1], msg_type=MessageTypes.ROUTE_REQUEST, payload=messagePayload)
