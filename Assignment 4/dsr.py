...
def discover_route(destination: str, routing_table: dict):
    ...
    discovery_message = generate_message(
        destination, MessageTypes.ROUTE_REQUEST, f"Route to {destination}"
    )
...
def send_routed(destination: str, message_payload: str, routing_table: dict):
    if destination not in routing_table.keys():
        ...
        send_message_to(generate_message(destination,MessageTypes.DATA,message_payload),destination)
